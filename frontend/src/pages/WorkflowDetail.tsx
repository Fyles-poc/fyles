...
import {
  ...
  Upload, Files, Heading1, AlertCircle, X, ShieldAlert,
} from 'lucide-react';
              ...
              <>
                ...
                <div className="absolute left-0 right-0 mt-2 bg-white border border-slate-200 rounded-xl shadow-lg z-20 overflow-hidden">
                  <div className="flex items-center justify-between px-4 py-2 border-b border-slate-100">
                    <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Type de champ</p>
                  ...
                  <div className="p-3 grid grid-cols-2 gap-4">
                    {groups.map((group) => (
                      <div key={group.label}>
                        <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-2 px-1">{group.label}</p>
                        <div className="space-y-0.5">
                          {group.fields.map(({ type, label, icon: Icon }) => (
                            <button
                              key={type}
                              onClick={() => addBlock(type)}
                              className="flex items-center gap-2.5 w-full px-2.5 py-2 text-sm text-slate-700 hover:bg-slate-50 rounded-lg transition-colors text-left"
                            >
